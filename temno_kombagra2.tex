...
Pro relaci $r$ definujeme inverzní relaci $r^{-1} = \{<y,x> | <x,y> \in r\}$.
\end{define}

\begin{define}
(Složení relací)

Pro relace $r,s$ definujeme jejich složení $r\circ s = \{<x,z> | <x,y> \in r\ \wedge\ <y,z> \in s\}$.
\end{define}
...
\[
	(\forall x \in D_f(f)) ((y\in H_f(f)) \ \wedge \ (y'\in H_f(f)) \ \wedge \ (<x,y> \in f)
...
Ostře uspořádaná množina je uspořádaná dvojice $<a,r>$, kde $a$ je množina a $r\subseteq a \times a$ je relace splňující:
\[
...
Počáteční úsek množiny $<a,r>$ je podmnožina $\{ y\ |\ (y \in a) \wedge (y\ r\ x)\}$.
...
Položíme 
\[
	f = \{<v,w>\ | \ v \in a\ \wedge \ w \in b\ \wedge \ (<(\leftarrow ,v), r> \ \cong \ < (\leftarrow , w),s >)
\]
Postupně ukážeme, že
\begin{enumerate}
\item $f$ je funkce	($<v,w> \in f$ a $<v,w_2> \in f$ pak 
\[
...
$ m = \{v \in a | (\forall w \in b) \ <v,w> \notin f\}$
a \\$o = \{w \in b | (\forall v \in a) \ <v,w> \notin f\}$
...
$\{ \{<u,v>| <u,v> \in s\}\ |\ \forall u \in D_f(s) \}$

$((3)\rightarrow(4)):$ definujeme relaci $s = \{<t,v> \ |\ (t\in x) \ \wedge\ (v \in y_t) \}$, dle $(3)$ existuje funkce $f$.
